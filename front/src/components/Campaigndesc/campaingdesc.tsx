...
const Campaingdesc = () => {
  ...
  const { userData } = useAuth(); // Obtenemos los datos del usuario desde el contexto de autenticación

  useEffect(() => {
    const fetchCampaignDetails = async () => {
      if (!userData?.userData.id) { // Verificamos que haya un ID de usuario
        setLoading(false);
        return;
      }
    ...
    if (userData?.userData?.id) { // Solo hacemos el fetch si tenemos el ID de usuario
