...
export class PaymentsService {
    ...
    constructor (private readonly userService: UserService , @InjectRepository(Account) private  repositoryAccount:Repository<Account>){
    ...
    async getFilteredPaymentMethods() {
      ...
      async createPayment(userId: string, packageId: number, payerEmail: string , paymentMethodId: string, installments: number) {
        const selectedPackage = await this.repositoryAccount.findOne({where:{id:packageId}});
