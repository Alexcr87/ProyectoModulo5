...
export class AccountSeeder {
    constructor(@InjectRepository(Account) private  accountService: Repository<Account>){}
    ...
    async seed() {
        ...
        for (const account of accounts){
            const existingaccount = await this.accountService.findOne({where:{name: account.name}});
